Plot.prototype.draw = function(frame) {
    ...
    this.frame = frame;
    if (this.ondraw) this.ondraw.call(this);
    return this;
}

Plot.prototype.toggle = function() {
...
Plot.prototype.labels = function(labels, anchor) {
    let k;
    for (k in labels) {
        let pos = labels[k];
        let opt = pos.length > 2 ? pos[2] : {};
        let img = Plot.images[k];
        let key = img ? "anchor" : "align";
        let opt0 = {};
        opt0[key] = anchor == null ? CENTER : anchor;
        Object.assign(opt0, opt);
        if (img == null) img = new Text(k, pos, opt0);
        else img = new Graphic(img, pos, opt0);
        this.items.push(img);
    }
}

Plot.prototype.barLabels = function(labels, offset, style, anchor) {
    let lbl = {};
    if (offset == null) offset = 0;
    if (typeof(offset) == "number") offset = [0.5, offset];
    for (let i=0;i<labels.length;i++) {
        let x = offset[0] + i;
        lbl[labels[i]] = [x, offset[1], style];
    }
    this.labels(lbl, anchor == null ? TOP : anchor);
}
...
function Graphic(img, pos, opt) {
    this.img = typeof(img) == "string" ? Plot.images[img] : img; //preload.get(img)
    this.pos = pos;
    if (opt == null) opt = {};
    this.opt = opt;
    if (opt.anchor == null)
...
function Text(text, posn, opt) {
    this.text = text;
    this.posn = posn;
    this.opt = {font:Plot.font, align:CENTER};
...
Segments.prototype.bar = function(x, y, width) {
// Set instance data to represent a single bar of a bar chart
    if (!width) width = 0.6;
    let w = width / 2;
    this.pts = [[x-w, 0], [x-w, y], [x+w, y], [x+w, 0]]; 
    this.isBar = true;
}

Segments.bar = function(x, y, width, opt) {
// Create a bar
    let opt0 = {fill:"blue", closed:true};
    if (opt) Object.assign(opt0, opt);
    let bar = new Segments([], opt0);
    bar.bar(x, y, width);
    return bar;
}

Segments.bars = function(data, offset, width, opt) {
// Create an array of bars
    let bars= [];
    if (offset == null) offset = 0.5;
    for (let i=0;i<data.length;i++) {
        bars.push(Segments.bar(i + offset, data[i], width, opt));
    }
    return bars;
}

Plot.prototype.barsData = function(data) {
// Modify data for bar chart
    let j = 0;
    for (let i=0;i<this.items.length;i++) {
        let item = this.items[i];
        if (item.isBar) item.pts[1][1] = item.pts[2][1] = data[j++];
    }
    return this;
}
...
function energyBarGraph(cv, data, tick, dp, unit) {
    for (let i=0;i<data.length;i++) {
        ...
        data[i] = [d[1], c, Plot.images[d[0]]]; // preload.get(d[0])
    }
...
energyBarGraph.label = {align:BOTTOM, font:Plot.font};
