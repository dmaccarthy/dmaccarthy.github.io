...
// Coordinate system methods...
	...
	extendCS(l, r, b, t, m) {
		...
		this.createCS(this.units, l, r, b, t, m);
	}
...
class BarGraph extends Sketch {
	...
	constructor(cv, n, yMax, dy, barWidth, margin) {
		...
		this.extendCS(l, r, 0, yMax, margin);
	...
	labels(labels, shift, scale, ...attr) {
		...
		let lbl = this.series("Label_", labels, x, 0, shift, {valign:"top"}, ...attr);
		if (scale) {
	...
	values(lbl, y, shift, ...attr) {
		...
		let opt = {align:"right", font:"bold 20px sans-serif", fill:"blue"};
		this.series("Value_", val, this.yAxis, y, shift, opt, ...attr);
		return this;
	}
